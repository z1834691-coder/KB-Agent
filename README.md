...
![Python](https://img.shields.io/badge/python-3.10%2B%20·%20stdlib--only-blue.svg)
...
> **📢 公开版说明**：本仓库是**净化导出版（sanitized export）**。为保护**闭卷评测（blind eval）**的有效性，**刻意不含评测"答案键"**——完整标注数据集与 gold 答案不公开，只公开被测 agent 可见的**题目包** `evals/02-challenge-dataset-v0.4-actor-pack.md`。被维护的知识库本体（Obsidian vault）及其管线 `kb_ops.py` 随私有仓库保存；本仓库文档中涉及它们的命令为**说明用法**。个人作品叙事、截图、第三方捆绑亦不在公开范围。"不公开答案键"本身就是本项目"评测不作弊"原则的体现。
...
> **依赖**：Python 3.10+、git。**零第三方依赖**（脚本纯标准库）。
> **公开导出说明**：本仓库的 `evals/robustness_audit.py` 可直接运行；而**库管线 `kb_ops.py`、完整盲评 harness（`run_blind_eval.py`/`flywheel_digest.py`）与 gold 答案键随被维护 vault 私有保存**——下列涉及它们的命令用于**说明用法与工作流**。想完整复现整套 loop，可按 [迁移 10 步](#迁移到你自己的知识库10-步)在你自己的库上搭起来。

```bash
# ① 客观机械审计（R1/R3/R4/R7，[MECH] only；故意拒绝给质量维度打分）—— 本仓库可直接跑
python3 evals/robustness_audit.py            # 人读报告
python3 evals/robustness_audit.py --json     # 机读 JSON

# ② 飞轮 digest → 下一轮聚焦建议（数据反哺选路）
python3 evals/flywheel_digest.py

# ③ 库体检（Doctor 机械结构分，只体检不改正文，产出周报 + dashboard）
python3 "<vault>/_meta/pipeline/kb_ops.py" doctor --force

# ④ 日常轻检 + 自主决策 + 重建 dashboard（--fetch 会拉官方源比对 hash）
python3 "<vault>/_meta/pipeline/kb_ops.py" run --fetch
...
python3 evals/run_blind_eval.py validate --run-dir evals/runs/<run_id>   # lint+summary+dashboard
...
完整决策日志见 [DECISIONS.md](DECISIONS.md)。几条最值得记的权衡：
...
我既是这个 agent 系统的 PM，也是它的用户。做它是为了回答一个我认为 AI PM 时代最核心的问题：**当模型能写任何东西时，"判断好坏 + 不被漂亮数字骗"就成了最稀缺的能力。** 这个项目就是这句话的可运行证明。
