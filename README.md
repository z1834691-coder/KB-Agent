...
```
                    ┌─────────────────────────────────────────────┐
                    │  进展.md (进度真相源) + ITERATION-TREE (节点树) │
                    └───────────────────────┬─────────────────────┘
                                            │ 每轮
   ┌──────────┐   ┌──────────────┐   ┌──────────────┐   ┌───────────────────────────┐   ┌──────────┐
   │ A 诊断    │→ │ B 选节点      │→ │ C 执行        │→ │ D 判官(judger)             │→ │ E 记录    │
   │ 机械审计  │   │ flywheel反哺  │   │ 单一变量      │   │ [MECH]脚本 + [JUDGE]独立LLM │   │ git+飞轮  │
   │ +doctor  │   │ +价值/成本    │   │ 禁注水        │   │ + 5条一票否决               │   │ +进展.md  │
   └──────────┘   └──────────────┘   └──────────────┘   └───────────────────────────┘   └────┬─────┘
        ↑                                                                                       │
        └───────────────────────── flywheel 数据反哺下一轮选路 ←──────────────────────────────┘
```
...
- Agent 行为分 v2→v5：8.3 → **8.94**（真实盲评，holdout gap 0.16）
- 知识库诚实分：初诊 4.2 →（诚实基线）6.6 → **7.4**（随真实深度自然升，非调分）
- 9 轮自主迭代，**5 条一票否决全 PASS、零注水**
- 沉淀 3 份 topic 无关 SOP + 一套可迁移评测框架 + 独立 LLM-Judge + 飞轮反哺
